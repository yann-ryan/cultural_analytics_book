...
![Examples of different geographic features found in shapefiles.](images/43_basemaps/01.png){width="600"}
...
Next, the data is at one of three scales:
...
**Finally, specify the `type`. For this, take the saved download link, and take everything after the ne\_ and the scale (in this case `ne_50m_`), and before the .zip. In this example this is simply `populated_places`.**
...
-   [ ] Know what a shapefile is, and what a simple features (sf) object looks like
-   [ ] Know what a Coordinate Reference System (CRS) is, at a basic level
-   [ ] Download a shapefile of countries or coastlines using `rnaturalearth`
-   [ ] Use `ne_download()` to find and download other Natural Earth data
