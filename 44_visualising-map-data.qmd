...
**Try it yourself:** Adjust the `xlim` and `ylim` values above to zoom the map in on Scandinavia instead.
...
Setting the coordinate limits doesn't change the data before visualisation. This means that the colour scale will be based on the entire map rather than the section used. If the data is mapped to something like population, this might mean that the map colours don't represent the full range of the data on the visible map, as here:
...
**Map projections** try to portray the surface of the earth, or a portion of the earth, on a flat piece of paper or computer screen. In layman's terms, a map projection transforms the earth from its spherical shape (3D) to a planar shape (2D).
...
![](images/44_visualising-map-data/01.png){width="500"}

### Accuracy

Map projections are never absolutely accurate representations of the spherical earth. As a result of the map projection process, every map shows some distortion of angular conformity, distance, or area - you can never preserve all three at once, so every projection is really a choice about which kind of distortion you're willing to accept.

## Changing projection

You can change the projection of a map object using `st_transform()`. The `sf` package supports over 170 different projections, which you can browse by running `sf_proj_info()` in the console.

To use one, add the correct string with `+proj=` to the `crs =` argument in `st_transform()`:
...
Now plot the transformed map - notice the shape of the world has changed to this new (Winkel Tripel) projection:
...
A compromise projection doesn't try to be perfectly accurate on any one property (area, angle, or distance) - instead it aims for a reasonable, visually-pleasing balance across all of them, which makes this type of projection well-suited to drawing a general-purpose visual map of the world. The Winkel Tripel projection above is one example; another is the Robinson projection:
...
A conformal projection preserves angles and shapes locally, which makes it useful for navigation and meteorology:
...
An equidistant projection preserves distances from one or two specific points to all other points, which is useful for navigation and for seismic mapping (for example, plotting distance from an earthquake's epicentre):
...
An equal-area projection preserves the relative size of areas, at the expense of distorting their shape. This matters a lot for data mapping specifically: if you're colouring countries or regions by some statistic (a choropleth map, as covered in the Digital Mapping overview), an equal-area projection avoids visually over- or under-stating the importance of a place purely because of the projection you happened to choose.

![](images/44_visualising-map-data/02.png){width="400"}
...
![](images/44_visualising-map-data/03.png){width="400"}

#### Projected CRS

A projected CRS instead finds points based on a flat, projected map rather than the globe itself. The most popular is the Universal Transverse Mercator (UTM) system, which divides the globe into a series of zones - to specify a point, you need to know which zone it falls in, as well as its position within that zone, given as two values called northing and easting.

![](images/44_visualising-map-data/04.png){width="400"}

#### What does this mean for mapping?

Merging or mapping multiple datasets requires them all to be in the same CRS. Sometimes this means setting or transforming the existing CRS.
...
-   [ ] Use regular tidyverse verbs (`filter()`, `mutate()`, `summarise()`) on an sf object
-   [ ] Draw a map with `geom_sf()`, and map a variable to its fill colour
-   [ ] Zoom in on part of a map with `coord_sf()`, and know why filtering and zooming aren't the same thing
-   [ ] Know what a map projection is, and change one with `st_transform()`
-   [ ] Know the difference between a geographic and a projected CRS
