...
You can also combine multiple select instructions. This will select the first to the third columns, and also the eighth column:
...
You can also select columns using more complicated methods. There are a group of 'tidyselect' helpers to select columns based on more complicated criteria. The most useful are:
...
First, starts_with() will look for any columns which start with some chosen text. It works as the following:

```{webr-r}
select(titanic_df, starts_with("s")) 
```

**Try it yourself:** In the cell below write a select function using starts_with() which selects the `parch` column, but **not** the `pclass` column.
...
To rename columns, we can use the `rename()` function with the syntax `rename(dataset, new_name = old_name)`. For example, to rename the `name` column to `passenger_name` in the Titanic dataframe, we can use:
...
-   [ ] Use `select()` to keep or drop columns by name or position
-   [ ] Select a range of columns, and combine multiple select instructions
-   [ ] Use tidyselect helpers: `starts_with()`, `ends_with()`, `contains()`
-   [ ] Rename columns with `rename()` (or while selecting)
