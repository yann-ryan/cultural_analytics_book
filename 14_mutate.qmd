...
![Source: https://allisonhorst.com/r-packages-functions](images/14_mutate/01.png){fig-alt="Cartoon of cute fuzzy monsters dressed up as different X-men characters, working together to add a new column to an existing data frame. Stylized title text reads \"dplyr::mutate - add columns, keep existing.\"" width="600"}
...
-   [ ] Use `mutate()` to create a new column from existing ones
-   [ ] Use an existing function (e.g. `round()`) inside `mutate()`
-   [ ] Use `if_else()` for a two-way condition, and `case_when()` for more than two
