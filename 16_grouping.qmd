...
-   [ ] Use `group_by()` to split a dataframe into groups before summarising
-   [ ] Group by more than one column at once
-   [ ] Use `group_by()` together with a `slice_` function and with `mutate()`, not just `summarise()`
-   [ ] Know when and why to `ungroup()` a dataframe
-   [ ] Use `count()` as a shortcut for `group_by()` + `summarise(n = n())`
