...
![Source: <https://allisonhorst.com/r-packages-functions>](images/12_filter/01.png){fig-alt="Cartoon illustrating dplyr::filter(): a fuzzy monster points at a map showing which rows of a data table (type, food, site) satisfy the condition type == \"otter\" & site == \"bay\", keeping only the matching rows and discarding the rest." width="600"}
...
-   [ ] Use `%in%` to filter against a list or range of values
-   [ ] Negate a filter condition with `!`
