...
[![By Allison Horst, CC-BY-4.0](images/23_ggplot2/01.png){width="500"}](https://github.com/allisonhorst/stats-illustrations?tab=readme-ov-file)
...
**We'll learn about the rest of the layers in upcoming sessions.**
...
For `geom_col`, we *have to* set `x` and `y` and we can *optionally* set `alpha` (the transparency), `colour` (the colour of the outline of the bars), `fill` (the colour of the inside of the bars), and `linetype` and `linewidth` (allowing you to customise the look of the outline further).
...
We learn a bit more about scales later this week. Turn to the next page to find out about more chart types and their most common attributes.
...
-   [ ] Know the essential layers of a ggplot chart: data, aesthetics, and geometry
-   [ ] Build a bar chart from scratch with `ggplot()` + `aes()` + `geom_col()`
-   [ ] Know the difference between setting an aesthetic inside `aes()` versus outside it
-   [ ] Know the difference between mapping a categorical vs a continuous variable to colour/fill
