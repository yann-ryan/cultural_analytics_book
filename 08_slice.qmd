...
**Try it yourself:** Use a slice\_ function to find the five **oldest** passengers (look at how we used `slice_min()` above to figure out how to do this).
...
-   [ ] Use `slice()` to subset rows by position, including ranges
-   [ ] Use `slice_min()` and `slice_max()` to select rows based on a numeric column
-   [ ] Know the difference between subsetting rows by position (`slice()`) and by condition (`filter()`, covered later)
