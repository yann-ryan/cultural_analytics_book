...
If you want a sum instead of a count of rows, give tally() a `wt` (weight) argument: `tally(wt = fare)` is the same as writing `summarise(n = sum(fare, na.rm = TRUE))`.
:::
...
-   [ ] Use `summarise()` to calculate a single summary value from a dataframe
-   [ ] Know the difference between counting rows (`n()`) and summing a value (`sum()`)
-   [ ] Know some other common summary functions (`mean()`, `median()`, `min()`, `max()`)
-   [ ] Use `tally()` as a shortcut for a simple count or sum
