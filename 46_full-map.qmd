...
::: callout-note
From this page on, the code cells are no longer 'live' - you can read them but not edit or run them in the book. This is because the shapefiles and datasets used from here on are too large, or need R packages that aren't available, to run in the browser. Follow along instead by opening the corresponding project in Posit Cloud, where all of this will run normally.
:::
...
::: {.callout-note}
The example `.gpkg` file for this section (a Dutch neighbourhood-boundary dataset) isn't included in this repo — add your own geopackage file and re-enable these chunks (remove `eval: false`) if you want to run them live.
:::
...
-   [ ] List the steps of a full map-making workflow, from raw data to finished map
-   [ ] Know the difference between shapefiles, geoJSON, and geopackage formats
-   [ ] Find a common key column to join a dataset to a shapefile
-   [ ] Join a dataset to a shapefile with `left_join()`, and map the result
