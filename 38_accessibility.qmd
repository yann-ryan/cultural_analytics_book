...
![](images/38_accessibility/01.png){fig-alt="A man walking with his partner turns his head to look at another person, symbolising a web user’s shift from inaccessible to accessible content. The original partner, representing inaccessible content, is ignored in favour of the accessible option, which is more appealing and inclusive." fig-align="center" width="500"}

### **What does it mean to make our content accessible?**

When making visualisations it is important to take into account accessibility. In this context, **accessibility means making the content more readable and understandable by everyone.**
...
-   Colour schemes that are not accessible to people who are colour blind or have low vision

-   Interactive elements that are not reachable for people navigating via keyboard.

### Color-blindness

Colour blindness (or colour vision deficiency, CVD) is the decreased ability to see colour or differences in colour, specifically hue.
...
In order to understand how colour in data visualisation is perceived by those with CVD, we should think back to the previous week's lesson on the differences between scales. The three key scales, as you'll remember, are **sequential** (for continuous data), **categorical** (for text or categories), and **diverging** (which is sequential, but with a 'midpoint'). These are affected in different ways.

#### Sequential colour scales

**Sequential colour scales** are affected the least, because they are generally made up of a single hue. However, the default blue colour scale provided by ggplot is not terribly good for those with general sight impairments - the 'Viridis' colour palette, which we used earlier in the course, is a good alternative choice as it is easier to distinguish differences. Another option is to apply a gradient scale, running from one distinct colour to another.

![](images/38_accessibility/02.png){width="400"}
...
A special type of gradient is a 'diverging' colour palette, where a third colour is used as a midpoint.

In both these cases, we should be mindful of using problematic colour pairs. Red-green diverging, in particular, should be replaced with another colour scale.

![](images/38_accessibility/03.png){width="450"}

**Try it yourself:** How can you make this chart of the US election results in 2020 more accessible?

::: callout-note
This example is a map, made with `geom_sf()` - a geometry we haven't covered yet (you'll meet it properly in the Digital Mapping part of the course). Don't worry about understanding that part of the code for now; just focus on the `scale_fill_gradient2()` line at the end, which is the part relevant to this exercise.
:::
...
Finally, categorical colour schemes (where the data is unordered, usually for categories or text), use a palette of distinct colours specifically chosen to be safe for colour vision deficiency. You can find a set of built-in palettes on the [ColorBrewer website](https://colorbrewer2.org/). Select 'qualitative' and tick the 'color-blind safe' checkbox. The three remaining palettes are 'Dark2', 'Set2', and 'Paired'.

**Try it yourself:** To choose a specific palette for a categorical value (such as species, in this case), use `scale_color_brewer()` and specify the palette from the list of colour blind safe above.
...
Another important thing to take into account is contrast. Contrast can be defined as the difference in perceived luminance between two colours. Higher contrasts are easier to perceive for everybody, particularly those with sight impairments.
...
Again, think about this when designing colour schemes.

For instance, try not to use colours with a low contrast ratio, particularly against a white background. By default, ggplot2 uses a grey background, but often white is a requirement or desired for design choices.
...
![](images/38_accessibility/04.webp){width="500"}
...
![](images/38_accessibility/05.png)
...
-   [ ] Explain what makes a data visualisation more (or less) accessible
-   [ ] Choose a colour-vision-deficiency-safe scale for sequential, diverging, and categorical data
-   [ ] Know why colour contrast matters, and how to improve it
-   [ ] Write good alt-text for a chart, and add it to a code cell
