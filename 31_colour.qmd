...
-   [ ] Know when ggplot picks a continuous vs a discrete colour/fill scale
-   [ ] Apply a built-in scale (e.g. viridis) to a chart
-   [ ] Build a scale from an existing palette (`scale_fill_distiller()`/`scale_fill_brewer()`)
-   [ ] Build a custom scale with specific colours (gradient/manual)
-   [ ] Adjust a size scale, and a legend's limits, breaks, and orientation
